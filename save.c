...
double save(void)
{
	...
	for(int i = 0;i <max_cnt;i++)	//找到要存储的账号
	{
		if(0 == strcmp(ctos.acc.id,temp[i].id))	//找到了要存储的ID
		{	
			temp[i].balance = temp[i].balance + ctos.acc.balance; //将金额加上
...
int main(int argc,const char* argv[])
{
	...
	for(;;)
	{
		...
		//double ret = query();
		sprintf(stoc.info,"存储成功，你当前的余额为：%lf\n",save());
		//返回的路径
		stoc.type = ctos.return_type;
		//发送
		send_msg(msgid,&stoc,sizeof(MSG_STOC));
